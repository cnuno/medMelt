...
void loadImages()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, 
            GL_RGBA, GL_UNSIGNED_BYTE,opData);
    free(opData);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, 
            GL_RGBA, GL_UNSIGNED_BYTE,exData);
    free(exData);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, 
            GL_RGBA, GL_UNSIGNED_BYTE,fieldData);
    free(fieldData);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, 
            GL_RGBA, GL_UNSIGNED_BYTE,staryData);
    free(staryData);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, 
            GL_RGBA, GL_UNSIGNED_BYTE,etData);
    free(etData);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, 
            GL_RGBA, GL_UNSIGNED_BYTE,octData);
    free(octData);
}
/*int check_keys(XEvent *e, Game *game)
  {
...
switch (game->render) {
    case ZACK:
        if (game->level2.player2.jumpCount < game->level2.player2.JUMP_MAX &&
                game->level2.player2.action != GROUNDPOUND && 
                game->level2.player2.action != DASH) {
...
switch (game->render) {
    case ZACK:
        //if in the air
        if (game->level2.player2.action != MOVE && 
                game->level2.player2.action != GROUNDPOUND && 
                game->level2.player2.action != DASH && 
                !game->level2.player2.collision(&game->level2.platform[0])) {
            game->level2.player2.action = GROUNDPOUND;
...
switch (game->render) {
    case ZACK:
        if (game->level2.player1.jumpCount < 
                game->level2.player1.JUMP_MAX && game->level2.player1.action 
                != GROUNDPOUND && game->level2.player1.action != DASH) {
    ...
    case ZACK:
        if (game->level2.player1.action != DASH) {
            ...
            if (game->level2.player1.direction == RIGHT) {
                game->level2.player1.delta.x = 
                    game->level2.player1.dashStrength;
